...
一套用 **3D Coverflow 卡片剧场 + 公式详情页**重新组织的微积分与三角函数公式速查系统。

不再是死记硬背的清单，而是「**点开一张定义卡 → 进入完整公式网络**」的联动记忆体验。每张卡片背后藏着对应模块的全部公式——一个不漏，配合结构化记忆文本，帮你建立完整的公式网络。

**适合谁**：正在学微积分与三角函数、需要快速查阅公式并理解联动关系的学生。
